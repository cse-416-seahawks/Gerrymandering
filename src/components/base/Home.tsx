...
function Home() {
  ...
  const handleStateChange = (state: States) => {
    dispatch({
      type : "CHANGE_STATE",
      payload : {
        currentState : state
    ...
    <div className="Home">
      ...
      <div className="StateMap-content">
        <header className="StateMap-header">
            ...
            <StateMap selectedState={state[state.length-1].currentState} onStateSelection={handleStateChange} districtCoordinates={districtCoordinates} selectedDistrict={selectedDistrict}/>
            </div>
